...
if [[ $(echo $HOST | cut -d "." -f 1) == "MacBook-Pro" ]]; then
...
if [[ $HOST == 'arch-thinkpad' || $(echo $HOST | cut -d "." -f 1) == "MacBook-Pro" ]]; then
...
if [[ $(uname -r | sed "s/[^[:alpha:]]//g") == "arch" || $(echo $HOST | cut -d "." -f 1) == "MacBook-Pro" ]]; then
